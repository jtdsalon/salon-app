...
This document describes how the salon-app is configured **only via environment variables**: API base URL, image URLs, and Content-Security-Policy (CSP) for images. No hardcoded API or image URLs; update your `.env` files per environment.
...
- **API calls:** `src/services/api/networkClient.ts` uses `baseApiUrl` as Axios `baseURL`, so all API requests go to the configured server.
- **WebSocket:** `src/contexts/RealtimeContext.tsx` uses `getServerOrigin()` for the Socket.IO server URL.
- **Development fallback:** If `VITE_APP_BASE_URL` is not set in development, the config uses `http://localhost:3000/api` so API calls don’t hit the app origin (e.g. port 5173). A console warning is shown; set the env var and restart the dev server to use your intended API.

---

## How Image URLs Work

- **Relative paths from API:** When the backend returns a path (e.g. `/uploads/salons/.../file.jpg`), the app uses `getFullImageUrl(path)` from `@/lib/util/imageUrl`, which prepends the **server origin** derived from `VITE_APP_BASE_URL` (e.g. `http://localhost:3000`).
- **Full URLs from API:** If the backend returns a full URL, `getFullImageUrl()` returns it unchanged. CSP allows all `https:` origins for images, so S3/CDN URLs work without extra config.
...
<img src={getFullImageUrl(salon.logo)} />
```
...
- **Solution:** `vite.config.ts` uses a small plugin that:
  ...
  3. Replaces the placeholder `__CSP_IMG_ORIGINS__` in `index.html` with that value.

**Note:** `getCspImgOrigins` only supports **absolute** `VITE_APP_BASE_URL`. If you use a relative URL (e.g. `/api`), set `VITE_CSP_IMG_SRC` in env if you need specific image origins.
...
| `src/contexts/RealtimeContext.tsx` | Uses `getServerOrigin()` for WebSocket URL. |
...
3. For relative `VITE_APP_BASE_URL` (e.g. `/api`), configure **VITE_CSP_IMG_SRC** if you need specific image origins in CSP.
