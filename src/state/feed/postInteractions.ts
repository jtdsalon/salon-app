...
export const togglePostLike = (postId: string, pageId?: string | null) => ({
  type: T.TOGGLE_POST_LIKE,
  payload: { postId, pageId: pageId ?? null },
})
...
export const addPostComment = (postId: string, comment: string, pageId?: string | null) => ({
  type: T.ADD_POST_COMMENT,
  payload: { postId, comment, pageId: pageId ?? null },
})
...
export function* togglePostLikeSaga(action: { payload: string | { postId: string; pageId: string | null } }): Generator<any, void, any> {
  ...
  try {
    const response: any = yield call(togglePostLikeApi, postId, pageId)
    if (response?.status === HTTP_CODE.OK || response?.status === HTTP_CODE.CREATED) {
      const raw = response?.data?.data ?? response?.data
...
export function* addPostCommentSaga(action: {
  payload: { postId: string; comment: string; pageId?: string | null }
}): Generator<any, void, any> {
  try {
    const { postId, comment, pageId } = action.payload
    const response: any = yield call(addPostCommentApi, postId, comment, pageId ?? undefined)
    yield* handlePostResponse(response)
  } catch (err) {
    // Could add error toast
  }
