...
export function getFeedPostsApi(
  ...
  if (actorPageId) params.actorPageId = actorPageId;
...
export function togglePostLikeApi(
  postId: string,
  pageId?: string | null
): Promise<AxiosResponse<PostResponse>> {
  return networkClient().request({
    ...
    data: pageId ? { page_id: pageId } : undefined,
  })
}
...
export function addPostCommentApi(
  ...
  return networkClient().request({
    ...
    data: pageId ? { comment, page_id: pageId } : { comment },
  })
}
...
export function mapApiPostToFeedPost(raw: any): FeedPost {
  ...
  return {
    ...
    authorSalonId: raw.authorSalonId ?? raw.author_salon_id ?? null,
