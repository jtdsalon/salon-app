 ...
 *    const url = `${baseApiUrl}/users/profile`
 ...
 *    getFullImageUrl() uses the server origin from VITE_APP_BASE_URL.
 */
...
function createApiConfig() {
  ...
  // Development-only fallback when env is missing so API calls don't hit the app origin (e.g. 5173)
...
export function getCspContent(): string {
  ...
  // frame-ancestors is only valid in HTTP headers, not in meta; omit it to avoid console warning
  return `default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src ${imgSrc}; connect-src 'self' https: http://localhost:* ws://localhost:* wss://localhost:*; base-uri 'self'; form-action 'self'`
}
