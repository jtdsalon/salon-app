...
interface RealtimeProviderProps {
  ...
  /** JWT access token. When null/empty, connection is not established. */
  token: string | null;
  /** Salon ID to join for dashboard/booking updates (salon staff) */
  salonId?: string | null;
  /** Polling interval (ms) when WebSocket is unavailable. 0 = no polling. */
...
export function RealtimeProvider({
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      socket.on('connect', () => {
        if (!active) return;
        setConnected(true);
        if (salonId) socket.emit('join_salon', salonId);
      });
  ...
  }, [token, salonId, emitToHandlers]);

  /** Polling fallback when WebSocket is disconnected - emitToHandlers triggers subscription handlers */
  useEffect(() => {
  ...
  const joinSalon = useCallback((salonId: string) => {
    socketRef.current?.emit('join_salon', salonId);
  }, []);

  const leaveSalon = useCallback((salonId: string) => {
    socketRef.current?.emit('leave_salon', salonId);
  }, []);
