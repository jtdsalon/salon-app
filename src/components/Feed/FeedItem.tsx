...
import * as salonService from '@/services/api/salonService';
import {
  ExpandableText,
  PostActionsBar,
  CommentSection,
...
export const FeedItem: React.FC<FeedItemProps> = ({
  ...
  const actorPage = user?.pages?.find((page) => page.salonId === user?.salonId);
  const followPageApi = (salonService as { followPageApi?: (pageId: string) => Promise<any> }).followPageApi;
  const unfollowPageApi = (salonService as { unfollowPageApi?: (pageId: string) => Promise<any> }).unfollowPageApi;
  ...
  const currentUserId = currentUser?.id || (currentUser as { sub?: string } | null)?.sub;
  const isOwnPost =
    !!currentUser &&
