...
interface FeedComposerProps {
  ...
  currentUser: any;
  /** Salon page to post as (in salon-app, owner posts as page not personal) */
  actorPage?: SalonPageActor | null;
...
const FeedComposer: React.FC<FeedComposerProps> = ({ open, onClose, onSave, currentUser, actorPage, initialPost }) => {
  ...
  const handleUpload = (type: 'before' | 'after') => async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const prevPreviewUrl = type === 'before' ? previewUrlBefore : previewUrlAfter;

    // 1. Instant preview via object URL
  ...
  const handleSaveInternal = () => {
    ...
    const isPostingAsPage = !!actorPage && !initialPost;
    const userId = isPostingAsPage ? actorPage.pageId : (currentUser?.id || initialPost?.userId || 'me');
    const userName = isPostingAsPage ? actorPage.pageName : (currentUser?.firstName ? `${currentUser.firstName} ${currentUser.lastName}` : s.common.anonymous);
    const userAvatar = isPostingAsPage ? null : (getFullImageUrl(currentUser?.avatar) || null);
    onSave({
      id: initialPost?.id || Math.random().toString(36).substr(2, 9),
      userId,
      userName,
      userAvatar: userAvatar || undefined,
      userType: isPostingAsPage ? 'page' : 'customer',
