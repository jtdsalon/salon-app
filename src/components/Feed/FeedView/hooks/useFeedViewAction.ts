...
export function useFeedViewAction() {
  ...
  const displayedPosts = useMemo(() => (viewTab < 2 ? getFilteredBase() : []), [viewTab, getFilteredBase]);
  ...
  const loadMoreItems = useCallback(() => {
    if (viewTab === 2 || isLoadingMore || !hasMore) return;
    if (viewTab === 0) {
  ...
  const handleUpdateComment = useCallback(
    (postId: string, commentId: string, comment: string) => {
  ...
  return {
    ...
    displayedPosts,
    feed,
    loading: viewTab < 2 ? activeSlice.loading : false,
