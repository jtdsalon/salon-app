...
const BillingView: React.FC = () => {
  ...
  // Calculate statistics from appointments and payments
  ...
  const validate = () => {
    ...
    const cleanCvc = tempPayment.cvc.replace(/\D/g, '');
    if (!tempPayment.cvc || tempPayment.cvc === '•••') {
      // Allow default masked state
    } else if (cleanCvc.length < 3) {
      newErrors.cvc = 'CVC too short';
    }
  ...
  const handleSavePayment = async () => {
    if (validate()) {
      ...
      // Call API to process payment if needed
  ...
  return (
    <Box sx={{ pb: 10 }} className="animate-fadeIn">
      ...
      <Dialog
        ...
        onClose={() => { setIsPaymentDialogOpen(false); setErrors({}); }}
        maxWidth="sm"
        fullWidth
        PaperProps={{
          sx: {
      ...
      >
        <DialogTitle sx={{ p: 3, pb: 1 }}>
          <Stack direction="row" justifyContent="space-between" alignItems="center">
            <Typography variant="h6" sx={{ fontWeight: 900 }}>Edit Payment Method</Typography>
            <IconButton onClick={() => { setIsPaymentDialogOpen(false); setErrors({}); }} size="small">
              <X size={18} />
            </IconButton>
          </Stack>
        </DialogTitle>
