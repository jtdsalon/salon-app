                  ...
                  <ListItemText
                    ...
                    primary={
                      <Stack direction="row" justifyContent="space-between" alignItems="center">
                        <Typography component="span" sx={{ fontWeight: 800, fontSize: '13px', color: 'text.primary' }} noWrap>
                          {chat.type === 'group' ? chat.groupName : otherP?.name}
                        </Typography>
                        <Typography component="span" sx={{ fontSize: '10px', color: 'text.secondary', ml: 1 }}>
                ...
                <ListItem key={contact.id} disablePadding sx={{ mb: 0.5 }}>
                  <ListItemButton onClick={() => setSelectedContacts(p => p.includes(contact.id) ? p.filter(id => id !== contact.id) : [...p, contact.id])} sx={{ borderRadius: '12px' }}>
                    ...
                    <ListItemText
                      primaryTypographyProps={{ component: 'div' }}
                      secondaryTypographyProps={{ component: 'div' }}
                      primary={<Typography component="span" sx={{ fontSize: '14px', fontWeight: 800 }}>{contact.name}</Typography>}
                      secondary={<Typography component="span" sx={{ fontSize: '11px', color: theme.palette.secondary.main, fontWeight: 600 }}>{contact.handle}</Typography>}
                    />
